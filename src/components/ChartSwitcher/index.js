...
import { initChart, setChartSeriesData } from '../../sdk/helper';
import './index.css';
...
export function ChartSwitcher() {
  ...
  const switcherChartContainerRef = useRef(null);
  const dispatch = useDispatch();

  useEffect(() => {
    let timer;
    if (!chartLoading && switcherChartContainerRef.current) {
        setCalculateLoading(true);
        timer = setTimeout(() => {
            const { chart, series } = initChart({
                containerNode: document.getElementById(`switcherChartId-${activeCheckbox.value}`),
                chartOptions: {
                    width: switcherChartContainerRef.current.clientWidth,
                    height: 60,
                    rightPriceScale: {
                        visible: false,
                        scaleMargins: {
  ...
  }, [chartLoading, activeCheckbox.value, switcherChartContainerRef]);
        ...
        checkboxes.map(item => {
          ...
          return (
            ...
            >
              ...
              <div className={"chartSwitcher_checkbox__priceInfo"}>
                <span className={"chartSwitcher_checkbox__priceInfo--price"}>{87777.3450}</span>
                <span className={"chartSwitcher_checkbox__priceInfo--percent"}>{"+2.17%"}</span>
              </div>

              {
                (calculateLoading) ? (
                  <div
                    ref={switcherChartContainerRef}
                    className={`chartSwitcher_checkbox_chart ${activeCheckbox.value === item.value ? 'chart_show' : ''}`}
                    id={`switcherChartId-${item.value}`}
                  >
                    <Loading />
                  </div>
                ) : (
