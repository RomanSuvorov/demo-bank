...
import { initChart, setChartSeriesData, tooltipHandler } from '../../sdk/helper';
import './index.css';
...
export function CryptoChart() {
  const [chartRender, setChartRender] = useState(null);
  const [chartSeries, setChartSeries] = useState(null);
  const [calculateLoading, setCalculateLoading] = useState(true);
  const [tooltip, setTooltip] = useState({ show: false, style: {} });
  const { chartLoading, chart, chartError } = useSelector(state => state.app);
  const { activeCheckbox, dataset, time } = chart;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const { chart, series } = initChart({
        containerNode: document.getElementById('cryptoChart_container'),
        chartOptions: {
          ...
          layout: {
            ...
            textColor: color.colorSecondaryText,
            fontSize: 12,
          },
          grid: {
            horzLines: {
        ...
        areaOptions: {
          topColor: 'rgba(246, 165, 8, 0.5)',
          bottomColor: 'rgba(246, 137, 8, 0.05)',
          lineColor: 'rgba(246, 165, 8, 1)',
          lineWidth: 2,
        ...
        withTooltip: true,
        tooltipStoreHandler: setTooltip,
        storeChart: chartRender,
      });

      setChartRender(chart);
    ...
    return () => {
      clearTimeout(timer);
      if (chartRender) {
        chartRender.unsubscribeCrosshairMove(
          (e) => tooltipHandler(
            e,
            chartSeries,
            document.getElementById('cryptoChart_container'),
            setTooltip
          )
        );
      }
  ...
  useEffect(() => {
    setChartSeriesData({ chart: chartRender, series: chartSeries, dataset });
  }, [dataset]);
  ...
  const handleSetTimePeriod = (e) => {
    ...
    dispatch(changeChartPeriod(+time));
  };

  return (
    <div className={"cryptoChart"}>
      {
        (calculateLoading || chartLoading) && (
      ...
      <div className={"cryptoChart_info"}>
        ...
        <div className={"cryptoChart_times"}>
          <div
            data-time={TIMES.min}
            className={`cryptoChart_times__item ${time === TIMES.min ? 'active' : ''}`}
            onClick={handleSetTimePeriod}
          >
            <span>{t('exchange.chart.times.first')}</span>
          </div>
          <div
            data-time={TIMES.hour}
            className={`cryptoChart_times__item ${time === TIMES.hour ? 'active' : ''}`}
            onClick={handleSetTimePeriod}
          >
            <span>{t('exchange.chart.times.second')}</span>
          </div>
          <div
            data-time={TIMES.day}
            className={`cryptoChart_times__item ${time === TIMES.day ? 'active' : ''}`}
            onClick={handleSetTimePeriod}
          >
            <span>{t('exchange.chart.times.third')}</span>
          </div>
        </div>
      </div>
      ...
      <div
        ref={chartContainer}
        className={"cryptoChart_container"}
        id={"cryptoChart_container"}
      >
        <div
          className={`cryptoChart_tooltip ${tooltip.show ? 'show' : ''}`}
          style={(tooltip.show && tooltip.style) ? tooltip.style : {}}
        >
          <div className={"cryptoChart_tooltip__date"}>
            <span>{tooltip.show ? tooltip.date : ''}</span>
          </div>
          <div className={"cryptoChart_tooltip__price"}>
            <div className={"cryptoChart_tooltip__dot"} />
            <span>{tooltip.show ? tooltip.price : ''}</span>
          </div>
        </div>
      </div>
    </div>
